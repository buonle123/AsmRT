      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.ScrollViewFlex}>
        <View style={[styles.InnerScrollView, { marginBottom: tabBarHeight }]}>
          <View style={styles.ItemContainer}>
            ...
            {CartList.length == 0 ? <EmptyListAnimation title='Empty' /> : <View style={styles.ListItemContainer}>
              {CartList.map((data: any) => <TouchableOpacity onPress={() => navigation.push('Details', {
                index: data
                  .index,
                id: data.id,
                type: data.type
          ...
          {CartList.length != 0 ? <PaymentFooter price={{ price: CartPrice, currency: '$' }} buttonTitle='Pay' buttonPressHandler={buttonPressHandler} /> : <></>}
        </View>
      </ScrollView>
    </View>
  )
